...
//
// Kosh is the context cache. It stores the KV cache (keys and values) for previously
// seen token sequences in a Radix Tree. When a new prompt shares a prefix with
// a cached prompt, Kosh injects the cached KV tensors into the runtime, bypassing
// the expensive prefill matrix math for those tokens.
//
// To bound memory usage, Kosh implements an O(1) LRU (Least Recently Used) eviction
...
public:
    ...
    KoshCache(size_t max_budget_bytes, int64_t n_layers, int64_t kv_dim);
