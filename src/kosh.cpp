...
KoshCache::KoshCache(size_t max_budget_bytes, int64_t n_layers, int64_t kv_dim)
    : max_budget_bytes_(max_budget_bytes), n_layers_(n_layers), kv_dim_(kv_dim) {
    root_ = std::make_shared<RadixNode>();
}

void KoshCache::mark_used(std::shared_ptr<RadixNode> node) {
...
void KoshCache::remove_leaf(std::shared_ptr<RadixNode> node) {
...
void KoshCache::evict_until_fits() {
...
int64_t KoshCache::find_longest_prefix(const std::vector<int64_t>& prompt_ids,
...
void KoshCache::commit(const std::vector<int64_t>& tokens,
