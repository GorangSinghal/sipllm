...
ModelConfig ModelConfig::from_source(const WeightSource& src) {
    ...
    if (c.arch_kind == Arch::Phi3) { c.block_spec.qkv_fused = true; c.block_spec.ffn_fused_gate_up = true; }
    if (first_int(src, {K("rope.dimension_count")}, v)) c.block_spec.rope_dim = v;
